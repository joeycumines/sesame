...
main() {
  ...
  client_name=genrest
  client_dir="$project_dir"/internal/"$client_name"
  ...
  mkdir -p "$client_dir"
  go -C "$project_dir"/hack/oapi-codegen tool oapi-codegen -old-config-style -generate types,client -package "$client_name" -o "$client_dir"/"$client_name".gen.go "$schema_dir"/"$schema_openapi"
  cd "$client_dir"
  go vet
  go build
  go test
