...
const (
	TunnelService_Tunnel_FullMethodName = "/sesame.v1alpha1.TunnelService/Tunnel"
	TunnelService_Stream_FullMethodName = "/sesame.v1alpha1.TunnelService/Stream"
)
...
func (c *tunnelServiceClient) Stream(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[StreamRequest, StreamResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &TunnelService_ServiceDesc.Streams[1], TunnelService_Stream_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type TunnelService_StreamClient = grpc.BidiStreamingClient[StreamRequest, StreamResponse]
...
func (UnimplementedTunnelServiceServer) Stream(grpc.BidiStreamingServer[StreamRequest, StreamResponse]) error {
...
func _TunnelService_Stream_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(TunnelServiceServer).Stream(&grpc.GenericServerStream[StreamRequest, StreamResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type TunnelService_StreamServer = grpc.BidiStreamingServer[StreamRequest, StreamResponse]
...
var TunnelService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "sesame.v1alpha1.TunnelService",
	...
	Metadata: "sesame/v1alpha1/tunnel.proto",
}
